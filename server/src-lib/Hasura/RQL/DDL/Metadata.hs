...
--           allowlist _ actions cronTriggers) = undefined -- do
  -- withPathK "tables" $ do
...
-- applyQP2 replaceMetadata = undefined -- do
  -- clearUserMetadata
...
--               schemas collections allowlist customTypes actions cronTriggers) = undefined -- do
...
fetchMetadataFromHdbTables defaultSourceConfig = liftTx do
  ...
  collections <- mapFromL _ccName <$> fetchCollections

  -- fetch allow list
  allowlist <- (HS.fromList . map CollectionReq) <$> fetchAllowlists
  ...
  where
    ...
    modMetaMap l f xs = do
      st <- get
      put $ foldr (\(qt, dfn) b -> b & at qt._Just.l %~ HM.insert (f dfn) dfn) st xs
    ...
    fetchCronTriggers =
      (mapFromL ctName . map uncurryCronTrigger)
